...
2. **工作中心**：集中处理话务员求助、未完成回访和到访确认。
3. **评分预览**：按通话目标试算话务员分数，优先处理需关注、逾期回访或低通话量坐席。
4. **学生管理**：新增/编辑学生，按姓名、电话、学校、状态、阶段、意向、地区筛选线索；支持批量导入、批量分配、自动均摊、学校分发和报名后状态维护。
5. **线索治理**：统一进入学生管理与分配、无效线索回收、多学校分发三个处理入口。
6. **话务员管理**：新增、编辑、停用、删除话务员；停用后不再分配新线索，删除时按系统规则回收其名下线索。
7. **报表查询**：通过汇总报表、趋势报表和通电量查询核对通话量、意向转化、报名转化和工作明细。
8. **系统设置**：维护 AI 分析引擎、模型密钥、PushPlus、拨号限制、回访提醒窗口、备份等运行配置。
...
4. 在学生详情里补充备注、登记回访、登记到访，必要时粘贴通话内容进行 AI 分析。
...
4. 进入学生详情后，可更新处理结果、跟进阶段、意向等级，查看完整时间线。
