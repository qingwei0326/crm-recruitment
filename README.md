...
1. **仪表盘**：查看线索总数、已联系、A 级意向、今日呼出和阶段分布，并从今日待办快速进入异常队列。
2. **每日运营闭环**：优先处理 A 级超时未推进、家访待处理、到校待确认、已报名未结算、话务员求助、分配后无通话、昨日遗留未联系和逾期回访；处理后在每日运营里标记状态和备注。
3. **工作中心**：按全部/求助/回访/到访队列集中处理事项，逾期回访和今日到访会优先展示。
4. **评分预览**：按通话目标试算话务员分数，优先处理需关注、逾期回访或低通话量坐席，并跳转到相关线索。
5. **学生管理**：新增/编辑学生，按姓名、电话、学校、状态、阶段、意向、地区筛选线索；支持批量导入、批量分配、自动均摊、学校分发和报名后状态维护。
6. **线索治理**：查看数据健康中心、疑似重复线索、风险提醒；进入学生管理、无效线索回收、多学校分发和重复手机号清理预览。
7. **账号管理**：查看管理员和话务员账号、任务归属、今日呼出、账号锁定状态；超级管理员可新增、编辑、解锁、重置密码和办理离职。
8. **报表查询**：报表中心会先给出管理结论，再通过汇总报表、趋势报表和通电量查询核对通话量、意向转化、报名转化和工作明细。
9. **操作记录与回滚**：按动作、分类、批次号搜索操作日志；分配批次可先预览再回滚，系统会跳过后续已重新分配的学生。
10. **系统设置**：维护 AI 分析引擎、模型密钥、PushPlus、拨号限制、回访提醒窗口、评分目标和备份等运行配置。

### 线索治理操作

- **数据健康中心**：集中查看重复手机号、同名同校同手机号、无手机号线索、已报名异常变更、A 级长期未跟进、分配后无通话、非工作时间状态变更。
- **疑似重复线索**：先看重复组和影响学生，再决定是否回到学生管理人工核对；重复手机号清理属于高危动作，需要超级管理员确认。
- **学生管理与分配**：适合日常查找、批量选择、手动调整归属、自动均摊和学校分发。
- **无效线索回收**：按无效原因和学校查看线索，可一键回收到未分配池，也可勾选后批量回收或删除。
- **多学校分发**：按学校批量选择未分配学生，支持自动均衡分发或指定话务员分发。
- 标记学生为“无效”时需要填写原因，便于后续回收和审计；删除、批量删除、按学校删除都应先核对影响范围。

### 话务员电脑端流程

1. 在 **待拨打** 列表按学校、阶段、意向、状态或关键词筛选学生。
2. 点击拨号按钮后，通话结束选择处理结果：已联系、未接、待回访、已报名、空号、无意向、孩子不想读或高分段。
3. 根据沟通情况更新跟进阶段、意向等级和分数；需要主管介入时标记“需协助”。
4. 在学生详情里查看完整时间线（分配、通话、备注、回访、家访、到校、意向变化），补充备注、登记回访、申请家访、预约到校，必要时粘贴通话内容进行 AI 分析。
