...
• 工作中心：集中处理求助、待回访和到访确认
• 评分预览：按通话目标试算坐席分数，识别需关注、逾期回访和低通话量坐席
...
标记学生为“无效”时需要填写原因，后续回收和审计会使用这条记录。`,
    },
    {
      title: '话务员管理',
...
2. 点击电话按钮拨号
3. 通话后选择快捷状态：已联系、未接、待回访、已报名或无效（标记无效会要求填写原因）
...
分析引擎由管理员在”系统设置 → AI 分析”中配置（DeepSeek / 小米 MiMo / 自定义）；未配置密钥时自动回退到关键词匹配，分析仍可出结果但精度较低。`,
    },
    {
      title: '回访和到访',
...
手机端学生详情中的回访和到访记录支持完成、改期或删除。`,
    ...
    {
      title: '学生详情和备注',
      content: `点击“学生详情”可查看学生基础信息、分配时间、意向等级、阶段和历史联系记录。
