...
### 🎨 界面优化
- **学生管理统计卡片** — “未分配线索”改为独立卡片展示，“跟进阶段分布”只保留真实跟进阶段，避免归属状态和阶段状态混用。

### 🔧 重构
- **路由模块化** — 继续拆分管理员、学生、统计和招生活动大路由，新增 `admin_*`、`students_*`、`stats_*`、`admissions_*` 子模块，保持原 API 路径不变。
- **后台管理拆分** — 配置、治理、每日运营、分配回滚、无效线索、账号管理等从 `admin.py` 拆出，降低单文件维护成本。
- **学生与招生流程拆分** — 学生导入、查询、电话、分配、报名，以及家访、到校、报名结算、工作项流程拆到独立路由。

### 🧪 测试与部署
- **前端测试提速** — Vitest 并发 worker 调整后，全量前端测试稳定在约 100 秒级。
- **安全部署闭环** — 多轮验证后推送 GitHub，并使用安全 Ubuntu 部署脚本上线；部署继续排除生产 `crm.db` 和 `.env`。
